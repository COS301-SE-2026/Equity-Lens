...
const ExposureStat = ({ label, value }) => (
  ...
  */
  ...
  <div
    ...
    {tickers?.length > 0 && (
      <p className="mt-1 font-mono text-[11px] tracking-wide" style={{ color: 'var(--text-secondary)' }}>
        {tickers.join(', ')}
      </p>
    )}
  </div>
 ...
 * }} props
 */
const DashboardHoldingsTable = ({ holdings, sectorData, marketContext, flashHoldings, flashSector, thresholds }) => {
  ...
  return (
    <div className="group relative">
      ...
      {holdings.length === 0 ? (
        <div className="flex flex-1 items-center justify-center p-8 text-center text-[13px]" style={{ color: 'var(--text-ghost)' }}>
        ...
        <div className="flex flex-1 flex-col gap-5 p-5 lg:flex-row">
          ...
          >
            ...
            <div className="mb-2.5 pb-2.5" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
            ...
            <div className="mb-1 flex w-full items-center gap-1">
              <div className="flex flex-1 items-center justify-between px-2">
                ...
                <div className="flex items-center gap-2.5 font-mono text-[11px] tracking-widest" style={{ color: 'var(--text-ghost)' }}>
            ...
            <div className="max-h-[420px] flex-1 space-y-0.5 overflow-y-auto [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }}>
              {sorted.length === 0 ? (
                <p className="py-4 text-center text-[12px]" style={{ color: 'var(--text-ghost)' }}>
                      ...
                      <div className="flex w-full items-center gap-1">
                        ...
                        >
                          <div className="min-w-0">
                            <div className="font-mono text-[13px] font-bold">{h.ticker}</div>
                            <div className="truncate text-[11px]" style={{ color: 'var(--text-ghost)' }}>{h.name}</div>
                          </div>
                          <div className="flex items-center gap-2.5 text-right">
                            <span
                              className={`${COL.risk} rounded px-1.5 py-0.5 text-right font-mono text-[11px] font-semibold tracking-widest`}
                              style={{ color: getConcRisk(weight, thresholds).color, background: 'var(--surface-hover)' }}
                              title={`${getConcRisk(weight, thresholds).label} concentration - ${weight.toFixed(1)}% of your book`}
                            >
                              {getConcRisk(weight, thresholds).label}
                            </span>
                            <span className={`${COL.weight} font-mono text-[12px]`} style={{ color: 'var(--text-ghost)' }}>{weight.toFixed(1)}%</span>
                            <span className={`${COL.today} font-mono text-[12px]`}title={todayTitle} style={{ color: dailyKnown ? (positive ? 'var(--signal-positive)' : 'var(--signal-negative)') : 'var(--text-ghost)' }}> {dailyKnown ? `${positive ? '+' : ''}${h.daily_change_pct.toFixed(2)}%` : '-'}
                              {localMove && <sup style={{ color: 'var(--text-ghost)' }}>{h.quote_currency}</sup>}
                            </span>
                            <Money as="span" className={`${COL.value} font-mono text-[13px] font-semibold`}>{zar(h.value ?? 0)}</Money>
                          </div>
                        </button>
        ...
        {marketContext?.available && contextSectors.length > 0 && (
          <div className="border-t px-5 pb-2 pt-4" style={{ borderColor: 'var(--border-subtle)' }}>
            <div className="mb-1 flex items-center gap-1.5">
              <p className="font-mono text-[11px] tracking-widest" style={{ color: 'var(--text-ghost)' }}>
