...
describe('Help page', () => {
  it('renders the page heading and intro copy', () => {
    ...
    expect(
      screen.getByText('Find guides, answers and useful resources for EquityLens.'),
    ).toBeInTheDocument();
  });
  ...
  it('renders every navigation card and resource link', () => {
    ...
    expect(screen.getByRole('link', { name: /Understand your Portfolio/i })).toHaveAttribute(
      'href',
      '/analytics',
    );
  ...
  it('links the resources to the template download and support mailbox', () => {
    ...
    expect(screen.getByRole('link', { name: /Portfolio Template/i })).toHaveAttribute(
      'href',
      '/template/EquityLens_Portfolio_Excel_Template.xlsx',
    );
    expect(screen.getByRole('link', { name: /Contact support/i })).toHaveAttribute(
      'href',
      'mailto:thebigfivetb5@gmail.com',
    );
  });
  ...
  it('expands an FAQ answer on click and collapses it again', async () => {
    ...
    expect(
      screen.getByText('Excel, using the template below, or a PDF statement from your broker.'),
    ).toBeInTheDocument();
  ...
  it('keeps only one FAQ answer open at a time', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Can the AI Assistant see my portfolio?' }),
    );

    expect(
      screen.getByText('Yes. Its answers are based on the portfolio you uploaded.'),
    ).toBeInTheDocument();
    expect(
      screen.queryByText('Excel, using the template below, or a PDF statement from your broker.'),
    ).not.toBeInTheDocument();
