...
const INDICATORS = {
  capm: {
    label: 'CAPM',
    tooltip:
      "The return you'd expect from this stock, given how much it tends to move with the market",
    ...
    plainFormula:
      'Starts with a safe baseline return, then adds extra expected return based on how much more the stock swings compared to the market.',
    formula: 'Expected Return = Risk-Free Rate + Beta x (Market Return - Risk-Free Rate)',
  },
  pe_ratio: {
    label: 'P/E Ratio',
    tooltip:
      "How much you're paying for every rand of profit the company makes. Below 15 is cheap; above 30 may be pricey unless it's a fast-growing company.",
    signal: (v) => (v < 15 ? 'positive' : v > 30 ? 'negative' : 'neutral'),
    describe: (v) =>
      v < 15 ? 'Below market average' : v > 30 ? 'Premium valuation' : 'In line with market',
    ...
    plainFormula:
      'Takes current share price and divides it by how much profit the company made per share over the last year.',
    formula: 'P/E Ratio = Share Price / Earnings Per Share (EPS).',
    sectorCaveat:
      'A "normal" P/E looks very different accross industries - banks, retail and tech companies. A lower P/E here doesn\'t mean better, just a lower price relative to that company\'s earnings.',
  },
  altman_z: {
    label: 'Altman Z',
    tooltip:
      'Score estimating how likely a company is to face financial trouble. Above 2.99 is safe; below 1.81 signals distress; in between is the grey zone.',
    ...
    formula:
      '1.2x(Working Capital/Assets) + 1.4x(Retained Earnings/Assets) + 3.3x(EBIT/Assets) + 0.6x(Market Cap/Liabilities) + 1.0x(Sales/Assets)',
    sectorCaveat:
      'Only fair to compare within similar industries. Better to check each stock against the base guidelines than rank holdings against each other',
  },
  sharpe: {
    label: 'Sharpe Ratio',
    tooltip:
      "How much you're getting relative to the ups and downs. Above 1.0 is good; negative means the risk isn't being rewarded.",
    ...
    describe: (v) =>
      v >= 1
        ...
        : v < 0
          ...
          : 'Modest return for risk',
    why: 'Lets you compare a smooth ride agaisnt a bumpy one fairly, instead of just looking at which stock ended up higher.',
    plainFormula:
      "Return above what you'd get risk-free, divided by how much the price bounces around.",
    formula: 'Sharpe = (Return - Risk-Free Rate) / Standard Deviation of Returns',
  },
  beta: {
    label: 'Beta',
    tooltip:
      'Measures how much this stock moves relative to the market. Above 1 means it swings more than the market.',
    signal: (v) => (v < 1 ? 'positive' : v > 1.5 ? 'negative' : 'neutral'),
    describe: (v) =>
      v < 1 ? 'Less volatile than market' : v > 1.5 ? 'Highly volatile' : 'Moves with the market',
  ...
  sortino: {
    label: 'Sortino Ratio',
    tooltip:
      'Like the Sharpe ratio but only counts the downside swings against the stock. Above 1 is good.',
    signal: (v) => (v >= 1 ? 'positive' : v < 0 ? 'negative' : 'neutral'),
    describe: (v) =>
      v >= 1 ? 'Good downside-adjusted return' : v < 0 ? 'Poor downside performance' : 'Moderate',
  ...
  rsi: {
    label: 'RSI',
    tooltip:
      'A short-term momentum gauge on a 0-100 scale. Above 70 is overbought; below 30 is oversold.',
    ...
    formula:
      'RSI = 100 - [100 / (1 + Average Gain / Average Loss)], typically over a 14-day window',
  },
};
...
const Tooltip = ({ text, children, align = 'left' }) => {
  ...
  return (
    ...
    >
      ...
      {show && (
        <span
          className="absolute bottom-full mt-2 w-64 rounded-lg px-3.5 py-3 pointer-events-none"
          style={{
...
const IndicatorCell = ({ indicatorKey, result, loading, onSelect }) => {
  ...
  if (!result || result.status === 'error')
    return (
      <button
        onClick={onSelect}
        className="flex flex-col gap-0.5 pt-1 text-left w-full cursor-pointer"
      >
        <span className="text-[11px] font-mono" style={{ color: 'var(--signal-negative,#ef4444)' }}>
          Error
        </span>
  ...
  if (result.status === 'insufficient_data')
    return (
      <button
        ...
        className="flex flex-col gap-0.5 pt-1 text-left w-full cursor-pointer"
      >
        <span className="text-[11px] font-mono" style={{ color: 'var(--text-ghost,#444)' }}>
          N/A
        </span>
  ...
  return (
    <button
      data-testid={`indicator-${indicatorKey}`}
      onClick={onSelect}
      className="flex flex-col gap-0.5 pt-1 text-left w-full cursor-pointer"
    >
      <span
...
const StockRow = ({
  ...
  >
    ...
    >
      <div
        className="w-8 h-8 rounded flex items-center justify-center text-[10px] font-mono font-bold flex-shrink-0"
      ...
      <button
        onClick={onEdit}
        aria-label={`Customize indicators for ${stock.ticker}`}
      ...
      {visibleKeys.map((key, i) => (
        <div
          ...
          style={{
            position: 'relative',
            overflow: 'visible',
            borderRight: i < visibleKeys.length - 1 ? '1px solid var(--border-subtle)' : 'none',
          }}
        >
          <div className="flex items-start justify-between gap-1 pt-3">
            <Tooltip
              text={INDICATORS[key].tooltip}
              align={i >= visibleKeys.length - 2 ? 'right' : 'left'}
            >
              <span
            ...
            <button
              onClick={() => onQuickRemove(key)}
              aria-label={`Remove ${INDICATORS[key].label}`}
          ...
          <IndicatorCell
            ...
            result={results?.[key]}
            loading={loading}
            onSelect={() => onCellSelect(key, stock.ticker)}
          />
        </div>
      ))}
    </div>
...
const IndicatorDetailModal = ({ indicatorKey, activeTicker, stocks, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="terminal-card w-full max-w-lg max-h-[85vh] overflow-y-auto"
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          <div>
            <p
              className="text-[9px] uppercase tracking-widest font-medium mb-1.5"
              style={{ color: 'var(--text-ghost,#444)' }}
            >
              Why it matters
            </p>
            <p
              data-testid="indicator-explanation"
          ...
          <div>
            ...
            <p
              className="text-[11px] leading-relaxed px-3 py-2.5 rounded"
              style={{
              ...
              <div className="flex flex-col gap-1">
                {comparisonRows.map((row) => (
                  <div
                    ...
                    style={{
                      background:
                        row.ticker === activeTicker
                          ? 'var(--border-subtle,#2a2a2a)'
                          : 'transparent',
                      border:
                        row.ticker === activeTicker
                          ? '1px solid var(--text-ghost,#444)'
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {customPresets.map((preset) => (
                  ...
                  >
                    <button
                      onClick={() => setDraft(preset.keys)}
                      className="text-[10px] font-mono pl-2.5 pr-1.5 py-1.5 cursor-pointer"
                      style={{
                        color: 'var(--text-secondary,#a0a0a0)',
                        background: 'transparent',
                        border: 'none',
                    ...
                    <button
                      ...
                      className="text-[10px] pr-2 cursor-pointer"
                      style={{
                        color: 'var(--text-ghost,#444)',
                        background: 'transparent',
                        border: 'none',
              ...
              {ALL_INDICATORS.map((key) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: isSelected ? 'var(--border-subtle,#2a2a2a)' : 'transparent',
                      border: isSelected
                        ? '1px solid var(--text-ghost,#444)'
                        : '1px solid transparent',
                    }}
                  >
                    <span
                      className="w-3.5 h-3.5 rounded flex items-center justify-center flex-shrink-0 mt-0.5"
                      style={{
                        border: '1px solid var(--text-ghost,#444)',
                        background: isSelected ? 'var(--text-primary,#e5e5e5)' : 'transparent',
                      }}
                    >
                      {isSelected && (
          ...
          <div>
            ...
            <div className="flex gap-1.5">
              ...
              <button
                ...
                style={{
                  border: '1px solid var(--border-subtle,#2a2a2a)',
                  color: canSavePreset ? 'var(--text-primary,#e5e5e5)' : 'var(--text-ghost,#444)',
          ...
          <div className="flex flex-col gap-2 pt-1">
            <label
              className="flex items-center gap-3 text-[10px] cursor-pointer"
              style={{ color: 'var(--text-secondary,#a0a0a0)' }}
            >
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              <button
                onClick={onClose}
                className="text-[10px] font-mono px-3.5 py-2 rounded cursor-pointer"
                style={{
                  color: 'var(--text-ghost,#444)',
                  border: '1px solid var(--border-subtle,#2a2a2a)',
                }}
...
export default function Analytics() {
  ...
  return (
    <>
      ...
      `}</style>
      <div
        className="p-4 flex flex-col gap-4 max-w-[1600px] mx-auto w-full"
      ...
      >
        ...
        >
          <div>
            <h1
              className="text-xs font-medium uppercase tracking-widest"
              style={{ color: 'var(--text-page,#e5e5e5)' }}
            >
              Analytics
            </h1>
            <p className="text-[10px] mt-0.5" style={{ color: 'var(--text-ghost,#444)' }}>
              How your holdings are doing - hover a label for a quick explanation, click a value to
              learn more
            </p>
            <Link
              to={ROUTES.AI_CHAT}
      ...
      {editingTicker && (
        <IndicatorPickerModal
          ...
          onSave={(keys) => {
            updateVisibleKeys(editingTicker, keys);
            setEditingTicker(null);
          }}
          onApplyToAll={(keys) => {
            handleApplyToAllHoldings(keys);
            setEditingTicker(null);
          }}
