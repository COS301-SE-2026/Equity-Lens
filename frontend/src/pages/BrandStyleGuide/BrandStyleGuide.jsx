    ...
    swatches: [
      {
        token: '--surface-base',
        usage:
          'Page background. Black in dark, white in light - the printed value follows whichever theme you are viewing this page in. Everything below is theme-invariant.',
      },
    ],
  },
  {
    ...
    swatches: [
      {
        token: '--text-page',
        usage:
          'Headings and body text sitting directly on the page canvas, outside any card. Dark-on-white in light mode, white-on-black in dark. Reach for this, not --text-primary, whenever there is no card behind the text - News rendered white-on-white in light mode because this pair was not discoverable here.',
      },
      {
        token: '--text-page-secondary',
        usage:
          'The secondary tier of the same pair - subtitles and supporting copy on the bare canvas.',
      },
    ],
  },
  {
    ...
    swatches: [
      { token: '--surface-raised', usage: 'Navbar, sidebar backing' },
      {
        token: '--surface-card',
        usage:
          'Flat opaque cards - terminal-card (dense tabular/ticker content, deliberately not glass)',
      },
      {
        token: '--surface-elevated',
        usage: 'Tooltips, dropdowns, hover/selected panels - sits above a card',
      },
      {
        token: '--surface-inset',
        usage: 'Recessed wells inside a card - inline code, input troughs',
      },
      {
        token: '--surface-hover',
        usage:
          'Row and list-item hover wash. White at low opacity, so it lifts whatever it sits on rather than replacing it',
      },
      { token: '--border-subtle', usage: 'Default dividers, card borders' },
      { token: '--border-mid', usage: 'Stronger borders, hover outlines' },
      {
        token: '--border-strong',
        usage: 'The heaviest hairline - focus containers, selected outlines',
      },
      {
        token: '--border-divider',
        usage: 'Faintest rule, for dividing rows inside an already-bordered card',
      },
    ],
  },
  {
    ...
    swatches: [
      ...
      { token: '--text-ghost', usage: 'Placeholder-level, decorative labels only' },
      {
        token: '--text-disabled',
        usage:
          "Disabled control text - deliberately below AA, matches a disabled control's de-emphasised role",
      },
    ],
  },
  {
    ...
    swatches: [
      {
        ...
        usage:
          'Primary buttons, active nav, links, focus ring, key stats - used sparingly (~10% of the UI)',
      },
      { token: '--accent-hover', usage: 'Accent hover/active state (darker, for press feedback)' },
      {
        token: '--accent-light',
        usage: 'Lighter accent tier for highlights/gradients - not a button or active-state colour',
      },
      {
        token: '--accent-subtle',
        usage: 'Accent wash for tinted backgrounds - selected rows, active chips',
      },
      {
        token: '--text-on-accent',
        usage: 'Text/icons on a filled accent (or filled-danger) surface',
      },
  ...
  },
  {
    ...
    swatches: [
      ...
      { token: '--signal-negative', usage: 'Losses, errors, destructive actions' },
      {
        ...
        usage: 'Warnings, moderate risk - distinct hue from the accent on purpose',
      },
      { token: '--signal-info', usage: 'Informational states only, optional' },
      {
        token: '--color-danger',
        usage:
          'A true alias of --signal-negative - one red for gains/losses and form validation. Filled danger buttons pair it with --text-on-accent (black), not white - see changelog',
      },
    ],
  },
  {
    ...
    swatches: [
      ...
      {
        token: '--signal-negative-rgb',
        usage: 'Bare channels - what FormInput composes its focus glow from',
      },
      ...
      {
        token: '--accent-primary-rgb',
        usage: 'Bare accent channels, for the same alpha-composition reason',
      },
    ],
  },
  {
    ...
    swatches: [
      ...
      {
        token: '--chart-tooltip-bg',
        usage: 'Backing for a chart tooltip - the elevated surface, so it reads above the card',
      },
      { token: '--chart-inactive', usage: 'Dimmed/unselected series' },
      {
        ...
        deprecated: true,
        usage:
          'Deprecated - zero var() usages left in the codebase. Portfolio series read --accent-primary directly now. Kept only so an old file referencing it still resolves; do not use it in new code.',
      },
      {
        token: '--chart-neutral',
        deprecated: true,
        usage:
          'Deprecated - its only remaining usage is the --chart-primary definition above. Secondary/benchmark series read --text-secondary directly now.',
      },
    ],
  },
];
...
const CONTRAST_ROWS = [
  ...
  {
    pair: 'text-secondary on the base glass tier',
    ratio: '8.50 : 1',
    target: 'AAA',
    status: 'pass',
    note: 'dark theme: 10.54:1. Was published as 6.95:1 / AA - computed against the older 0.78 glass and never redone when it went to 0.85',
  },
  {
    pair: 'text-dim on the base glass tier',
    ratio: '6.05 : 1',
    target: 'AA',
    status: 'pass',
    note: 'dark theme: 6.91:1. Was published as 5.13:1, same stale composite',
  },
  {
    pair: 'text-ghost on the base glass tier',
    ratio: '5.45 : 1',
    target: 'AA',
    status: 'pass',
    note: 'dark theme: 6.06:1. Was published as 4.67:1. Still decorative/placeholder-level by convention, happens to clear AA anyway',
  },
  {
    pair: 'text-disabled on the base glass tier',
    ratio: '2.68 : 1',
    target: 'n/a',
    status: 'warn',
    note: 'dark theme: 2.46:1. Deliberately low - disabled controls are not held to text contrast minimums',
  },
  {
    pair: 'text-page on the page canvas (light) / (dark)',
    ratio: '16.56 : 1',
    target: 'AAA',
    status: 'pass',
    note: 'dark theme: 18.80:1. Measured against --surface-base directly, NOT the glass composite - this pair exists precisely for text with no card behind it',
  },
  {
    pair: 'text-page-secondary on the page canvas',
    ratio: '5.74 : 1',
    target: 'AA',
    status: 'pass',
    note: 'dark theme: 9.40:1. Light is the tighter of the two here, since 60% black on white is lighter than 68% white on black is dark',
  },
  {
    pair: 'accent-primary on the base glass tier',
    ratio: '5.25 : 1',
    target: 'AA',
    status: 'pass',
    note: 'dark theme: 7.31:1',
  },
  {
    pair: 'signal-positive on the base glass tier',
    ratio: '8.06 : 1',
    target: 'AAA',
    status: 'pass',
    note: 'dark theme: 11.23:1',
  },
  {
  ...
  {
    pair: 'text-on-accent (black) on accent-primary bg (button)',
    ratio: '7.31 : 1',
    target: 'AAA',
    status: 'pass',
    note: 'no glass involved - solid on solid',
  },
  {
...
const TYPE_SCALE = [
  {
    ...
    weight: 700,
    lh: 1.1,
    usage: 'Hero headings, landing page',
  },
  ...
  },
  {
    ...
    weight: 400,
    lh: 1.5,
    usage: 'Captions, metadata, badges',
  },
  {
    name: 'Micro',
    size: '10px / 0.625rem',
    weight: 500,
    lh: 1.4,
    usage: 'Tags, dense dashboard labels',
  },
];
...
const GLASS_TOKENS = [
  {
    ...
    usage:
      'Base tier - the everyday card. Black smoked glass, 0.85 opacity, theme-invariant - only the page background switches white/black',
  },
  {
    tokens: ['--glass-bg-elevated', '--glass-border-elevated'],
    usage:
      'Elevated tier - anything floating over a base card: modals, dropdowns, tooltips. More opaque again (0.90) so it reads as clearly "above" the base tier',
  },
  {
    tokens: ['--glass-hover'],
    usage:
      'Hover fill for a glass control - .glass-control:hover. Opaque enough to register as a state change without the blur shifting underneath',
  },
  { tokens: ['--glass-blur'], usage: 'Shared backdrop-filter blur radius, same for both tiers' },
  {
    tokens: ['--glass-highlight'],
    usage:
      'Inset top-edge highlight line - the "light catching the glass" that separates real glass from a flat translucent rectangle',
  },
  {
    tokens: ['--duration-theme'],
    usage:
      'Slow cross-fade for the light/dark background swap - deliberately separate from the fast interactive-feedback durations, so the black<->white flip reads as gradual, not abrupt',
  },
  {
    tokens: ['--scrim'],
    usage:
      'Modal/sidebar-overlay backdrop dimming colour - pure black, theme-invariant (dims a white page and a black page equally well)',
  },
];
...
const MOTION_SCALE = [
  {
    ...
    value: '150ms',
    easing: 'ease-out',
    usage: 'Hover, focus, button press',
  },
  {
    name: 'Page / section transition',
    value: '300ms',
    easing: 'ease-out',
    usage: 'Route changes, reveal-on-scroll',
  },
  {
    name: 'Complex animation (max)',
    value: '500ms',
    easing: 'ease-out',
    usage: 'Multi-step or chained motion',
  },
];
...
const CHANGELOG = [
  ...
  },
  {
    area: 'Corner radius',
    change:
      'Unchanged this pass - cards stay at 16px (rounded-2xl), matching the glassmorphism direction already established on the dashboard.',
  },
  {
    area: 'Accessibility',
    change:
...
const SectionHeading = ({ id, eyebrow, title, blurb }) => (
  <div id={id} style={{ scrollMarginTop: '80px', marginBottom: '28px' }}>
    ...
    <h2
      style={{ fontSize: '28px', fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.3 }}
    >
      {title}
    </h2>
    {blurb && (
...
const StatusBadge = ({ status }) => {
  ...
  const map = {
    ...
    warn: {
      ...
      border: 'var(--signal-warning-border)',
    },
    fail: {
      label: 'Fails',
      bg: 'var(--signal-negative-bg)',
    ...
    >
      ...
      <div className="mx-auto max-w-6xl px-4 lg:px-6 py-12 space-y-24">
        <motion.section {...FADE_UP}>
          ...
          <h1
            style={{ fontSize: '48px', fontWeight: 700, lineHeight: 1.1, letterSpacing: '-0.02em' }}
          >
          ...
          >
            This is both the brand style guide and the design system for EquityLens, deployed as a
            living page rather than a static document. Every value shown below is read from the same
            tokens and components the app actually ships with -; if this page and the running app
            ever disagree, this page is wrong, not the app.
          </p>
        </motion.section>
        ...
        <motion.section {...FADE_UP}>
          ...
          <div className="space-y-10">
            ...
            >
              ...
              >
                These names still resolve, and you will meet them in older files. Each is an alias,
                not a separate colour - they are left out of the swatches above so the palette is
                not shown twice. Prefer the target on the right in new code.
              </p>
                ...
                {TOKEN_ALIASES.map((a) => (
                  ...
                  >
                    ...
                    <span style={{ color: 'var(--text-dim)' }}>
                      {' '}
                      &middot; {tokenValues[a.alias] || 'not defined'}
                    </span>
                  </p>
                ))}
              </div>
            ...
            {TYPE_SCALE.map((t, i) => (
              <div
                ...
                style={{
                  ...
                  padding: '18px 0',
                  borderBottom:
                    i < TYPE_SCALE.length - 1 ? '1px solid var(--border-subtle)' : 'none',
                  flexWrap: 'wrap',
                }}
              >
                <span
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              ...
              <p style={{ fontSize: '12px', color: 'var(--text-dim)', marginTop: '12px' }}>
                Stroke width 1.5px default, 2px for emphasis. Interactive icons use
                --accent-primary; inline icons match surrounding text colour. In practice dense
                dashboard UI now also uses 9&ndash;14px sizes below the original three-tier scale -;
                documented here rather than pretending it doesn&apos;t happen.
              </p>
            </div>
          </div>
        </motion.section>
        ...
        <motion.section {...FADE_UP}>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {RADIUS_SCALE.map((r) => (
                    ...
                    >
                      <p
                        style={{ fontSize: '11px', color: 'var(--text-primary)', fontWeight: 600 }}
                      >
                        {r.value}
                      </p>
          ...
          <div style={{ marginTop: '32px' }}>
            ...
            >
              ...
              these two tiers via the shared{' '}
              <code style={{ fontFamily: 'var(--font-mono)' }}>.glass-surface</code> /{' '}
              <code style={{ fontFamily: 'var(--font-mono)' }}>.glass-surface-elevated</code> CSS
              classes (see globals.css) rather than each component hand-styling its own
              translucency. Base is the everyday card; elevated is for anything floating over a base
              card - a touch more opaque, heavier shadow, so the depth reads correctly when one
              glass surface sits over another. Don&apos;t nest elevated-in-elevated or base-in-base
              - stacked translucency is what washes the effect out.
            </p>
            <div
              ...
              style={{
                ...
                borderRadius: '16px',
                background:
                  'linear-gradient(135deg, var(--signal-info) 0%, var(--accent-primary) 50%, var(--signal-positive) 100%)',
              }}
            >
            ...
            <div className="space-y-2" style={{ marginTop: '16px' }}>
              {GLASS_TOKENS.map((t) => (
                <div
                  key={t.tokens.join()}
                  style={{ display: 'flex', gap: '12px', fontSize: '12px', flexWrap: 'wrap' }}
                >
                  <span
                    style={{
                      fontFamily: 'var(--font-mono)',
                  ...
                  <span style={{ color: 'var(--text-secondary)', flex: 1, minWidth: '240px' }}>
                    {t.usage}
                  </span>
                </div>
              ))}
          ...
          <div style={{ marginTop: '32px' }}>
            <h3 style={{ fontSize: '13px', fontWeight: 600, marginBottom: '12px' }}>
              Breakpoints (Tailwind defaults, unmodified)
            </h3>
        ...
        <motion.section {...FADE_UP}>
          ...
          >
            ...
            --glass-bg, --surface-base or a text alpha changes - the composite is what the ratios
            depend on, not the token values on their own.
          </p>
          ...
          >
            <li>
              All interactive elements are reachable via Tab; focus uses a visible 2px
              accent-coloured outline (see *:focus-visible in globals.css)
            </li>
            <li>
              prefers-reduced-motion is respected globally -; animation/transition durations
              collapse to near-zero
            </li>
            <li>
              Colour is never the only signal: gain/loss always pairs with a +/&minus; sign or icon
            </li>
        ...
        <motion.section {...FADE_UP}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div
              style={{
                ...
                border: '1px solid var(--signal-positive-border)',
                borderRadius: '12px',
                padding: '16px',
              }}
            >
          ...
          >
            When discussing real, publicly traded securities (illustrative figures in the Simulator,
            drawdown examples), never state fabricated numbers with absolute certainty , label them
