...
describe('Landing', () => {
  it('renders hero headline and CTA', () => {
    ...
    expect(screen.getAllByRole('link', { name: /analyse my portfolio/i }).length).toBeGreaterThan(
      0,
    );
  ...
  it('switching presets updates the concentration summary', () => {
    renderPage();
    expect(screen.getByText(/no single company makes up more than 5%/i)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('radio', { name: /jse blue-chip portfolio/i }));
    expect(screen.getByText(/these five blue-chips make up under 40%/i)).toBeInTheDocument();
    expect(screen.queryByText(/no single company makes up more than 5%/i)).not.toBeInTheDocument();
  });
