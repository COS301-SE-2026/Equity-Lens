...
describe('marketDataService', () => {
    ...
    it('fetches stock details', async () => {
        vi.mocked(api.get).mockResolvedValueOnce({data: {ticker: 'AAPL'}});
    ...
    it('fetches historical data', async() => {
        vi.mocked(api.get).mockResolvedValueOnce({data: {symbol: 'AAPL'}});
    ...
    it('searches stock', async() => {
        vi.mocked(api.get).mockResolvedValueOnce({data: {results: [{ticker: 'AAPL', name: 'Apple Inc.'}]}});
