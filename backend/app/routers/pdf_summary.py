...
def import_get_summary_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
    return get_summary_import_PDF(database=db, portfolioID=portfolioID, user_id=CurrentUser.id)


@router.get("/top_holdings/{portfolioID}", response_model=list[HoldingResponse])
def import_get_the_top_holdings_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
    return get_the_top_holdings_import_PDF(
        database=db, portfolioID=portfolioID, user_id=CurrentUser.id
    )


@router.get("/lowest_holdings/{portfolioID}", response_model=HoldingResponse)
def import_get_the_lowest_holdings_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
...
def import_get_the_top_allocation_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
...
def import_get_trading_activity_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
...
def import_get_cash_flow_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
    return get_cash_flow_import_PDF(database=db, portfolioID=portfolioID, user_id=CurrentUser.id)


@router.get("/dividend_income/{portfolioID}", response_model=list[DividendResponse])
def import_get_dividend_income_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
...
def import_get_expenses_import_PDF(
    portfolioID: str,
    db: Session = Depends(get_db),
    CurrentUser: UserResponse = Depends(get_current_user),
):
    return get_expenses_import_PDF(database=db, portfolioID=portfolioID, user_id=CurrentUser.id)
