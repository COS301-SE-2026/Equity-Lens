...
def test_pe_ratio_uses_direct_calculation_for_positive_eps():
    with (
        patch(
            "app.services.indicator_service.get_cached_price_history", return_value=_price_history()
        ),
        patch(
            "app.services.indicator_service.get_cached_fundamentals",
            return_value={
...
def test_no_price_data_produces_whole_row_error():
    with patch(
        "app.services.indicator_service.get_cached_price_history", return_value=pd.DataFrame()
    ):
